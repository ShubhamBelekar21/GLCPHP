...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-8">
                <h5 class="text-primary nav-link active">Infrastructure Details</h5>
            </div>

            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreateInfrastructure') }}"
                   style="color:white;">
                    <b class="">Create</b> </a>
            </div>
        </div>
...
{{--                <th></th>--}}
            ...
            @foreach($InfrastructureList as $item)
                <tr>
                    <td id="pi" scope="row">{{ $item->Title }}</td>
                    <td>{{ $item->Description}}</td>
{{--                    <td><img src={{$item->Photo1}} height="100" width="100"></td>--}}
{{--                    <td><img src={{$item->Photo2}} height="100" width="100"></td>--}}
                    <td>{{ $item->IsActive }}</td>
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditInfrastructure',array($item->SrNo))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewInfrastructure',array($item->SrNo))}}">| View </a>
...
@section('selectize-script')
    ...
    <script type="text/javascript">
        function changestatus(SrNo, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/InfrastructureStatusChange')}}/" + SrNo + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
