    ...
    <div class="mwb-form-main-container">
        <div class="row">
            <div class="" style="margin: 10px; text-align: center;">
                <h2 class="text-primary">Edit Infrastructure Details</h2>
            </div>

        </div>

        {{Form::open(array('action' => array('App\Http\Controllers\GLCMasterController@PostEditInfrastructure'),'method' => 'post'))}}
        ...
        <div class="mwb-form-group">
            <label for="name" class="">Upload Photo 1</label>
        ...
        {{--        for Photo2--}}
        <div class="mwb-form-group">
            <label for="name" class="">Upload Photo 2</label>
            <input type='file' id="Photo2" name='Photo2' class="form-control">
        </div>
        <div class="mwb-form-group" style="margin-left:230px; margin-top: 20px;">
            <div class="" style="margin-top: 0.8%;">
                @if ($Photo2 == null || $Photo2 == "")
                    <img id="Photo2" style="width: 100px; height: 100px; border: solid;" src=""/>
                @else
                    <img id="Photo2" style="width: 140px; height: 140px; border: solid;"
                         src="{{ asset($Photo2) }}"/>
                @endif
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#btnsubmitid').click(function () {
            ...
            var Photo2 = $('#Photo2')[0].files;
            var Title1 = $("#Title").val();
            var Description1 = $("#Description").val();
            var SrNo1 = $("#SrNo").val();

            if (files != null && files != "" || Photo2 != null && Photo2 != "" || Title1 != null && Title1 != "" ||
                Description1 != null && Description1 != "" ) {
                var fd = new FormData();

                fd.append('file', files[0]);
                fd.append('Photo2', Photo2[0]);
                ...
                // AJAX request
                $.ajax({
                    url: "{{route('PostEditInfrastructure')}}",
                    method: 'post',
                    data: fd,
                    contentType: false,
                    processData: false,
                    ...
                    success: function (data) {
                        debugger;
                        window.location.href = '/InfrastructureIndex2';
                    }
                });
            }
        });
