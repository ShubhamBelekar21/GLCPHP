...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-8">
                <h5 class="">Button Details</h5>
            </div>

            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreateButtonsDetails') }}"
                   style="color:gray;">
                    <b class="">Create</b> </a>
            </div>
        </div>
    ...
    <div class="table-responsive card" style="padding:10px;">
        ...
        width:100%; text-align: center;">
            ...
            <tbody>
            @foreach($FourButtonIndexList as $item1)
                <tr>
                    <td id="pi" scope="row">{{ $item1->id }}</td>
                    <td>{{ $item1->DisplayText }}</td>
                    <td>{{ $item1->PdfUrl }}</td>
{{--                    <td>{{ $item1->Url }}</td>--}}
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditButtonDetails',array($item1->id))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewButtonDetails',array($item1->id))}}">| View |</a>

                        @if( $item1-> IsActive == '1' )
                            <a class="text-primary" href="#" onclick="changestatus('{{$item1->id}}','0')">DeActive</a>
                        @Else
                            <a class="text-primary" href="#" onclick="changestatus('{{$item1->id}}','1')">Active</a>
                        @endif
                    </td>
                </tr>
            @endforeach
...
@section('selectize-script')
    ...
    <script type="text/javascript">
        ...
        function changestatus(id, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/FourButtonStatusChange')}}/" + id + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
                    success: function (data) {
                        ...
                        // window.location.href = '/GLCMaster/FourButtonIndex'
                        // window.location.href = '/SalaryGeneration';
