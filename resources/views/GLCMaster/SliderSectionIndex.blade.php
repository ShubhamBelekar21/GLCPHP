...
@section('page-css')
    ...
    <style>
        ...
        .bttn {
            font-size: 20px;
            padding: 10px 20px;
            background-image: linear-gradient(315deg, Grey 0%, Grey 74%);
            color: white;
            line-height: 50px;
            border: none;
            margin-top: -50px;
...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-8">
                <h5 class="text-primary nav-link active">Slider Details</h5>
            </div>

            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreateSliderDetails') }}"
                   style="color:gray;">
    ...
    <div class="table-responsive card" style="padding:10px;">
        ...
        width:100%; text-align: center;">
            ...
            <tbody>
            @foreach($SliderSectionList as $item)
                <tr>
                    <td id="pi" scope="row">{{ $item->id }}</td>
                    <td>{{ $item->Title}}</td>
                    <td>{{ $item->ImageUrl}}</td>
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditSliderDetails',array($item->id))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewSliderDetails',array($item->id))}}">| View |</a>
...
@section('selectize-script')
    ...
    <script type="text/javascript">
        function changestatus(id, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/SliderStatusChange')}}/" + id + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
                    success: function (data) {
                        debugger;
                        window.location.href = '/SliderSectionIndex';
                    },
                    error: function (ex) {
                        console.log(ex);
                    }
