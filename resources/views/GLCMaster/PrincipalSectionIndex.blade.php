...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-8">
                <h5 class="text-primary nav-link active">Principal Section Details</h5>
            </div>

            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreatePrincipalDetails') }}"
                   style="color:white;">
                    <b class="">Create</b> </a>
            </div>
        </div>
    ...
    <div class="table-responsive card" style="padding:10px;">
        ...
        width:100%; text-align: center;">
            ...
            <tbody>
            @foreach($PrincipalSectionList as $item)
                <tr>
                    ...
                    <td>{{ $item->ImageUrl }}</td>
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditPrincipalDetails',array($item->id))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewPrincipalDetails',array($item->id))}}">| View |</a>
...
@section('selectize-script')
            ...
            $('#dt').DataTable({
                ...
                initComplete: function () {
                    this.api().columns().every(function () {
                        var column = this;
                        var select = $('<select style="width: 100px;"><option value="">All</option></select>')
    ...
    <script type="text/javascript">
        function changestatus(id, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/PrincipalStatusChange')}}/" + id + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
                    success: function (data) {
                        debugger;
                        window.location.href = '/PrincipalSectionIndex';
                    },
                    error: function (ex) {
                        console.log(ex);
                    }
