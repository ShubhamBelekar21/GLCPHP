...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-8">
                <h5 class="text-primary nav-link active">Gallery Details</h5>
            </div>

            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreateGalleryDetails') }}"
                   style="color:white;">
                    <b class="">Create</b> </a>
            </div>
        </div>
    ...
    <div class="table-responsive card" style="padding:10px;">
        ...
        width:100%; text-align: center;">
            ...
            <tbody>
            @foreach($GallerySectionList as $item)
                <tr>
                    <td id="pi" scope="row">{{ $item->id }}</td>
                    <td>{{ $item->Title}}</td>
                    <td>{{ $item->ImageUrl }}</td>
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditGalleryDetails',array($item->id))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewGalleryDetails',array($item->id))}}">| View |</a>
...
@section('selectize-script')
    ...
    <script type="text/javascript">
        function changestatus(id, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/GalleryStatusChange')}}/" + id + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
