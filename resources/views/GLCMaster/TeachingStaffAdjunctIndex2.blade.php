...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-8">
                <h5 class="text-primary nav-link active">Teaching Staff Adjunct Details</h5>
            </div>

            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreateTeachingStaffAdjunct') }}"
                   style="color:white;">
                    <b class="">Create</b> </a>
            </div>
        </div>
    ...
    <div class="table-responsive card" style="padding:10px;">
        ...
        width:100%; text-align: center;">
            ...
            <tr>
                ...
                <th scope="col">NameoftheProfessors</th>
                <th scope="col">Subject</th>
                <th scope="col">Qualification</th>
                <th scope="col">TeachingExperience</th>
                <th scope="col">ProfessionalExperience</th>
                <th scope="col">Research_Publication</th>
            ...
            <tbody>
            @foreach($TeachingStaffAdjunctList as $item)
                <tr>
                    <td id="pi" scope="row">{{ $item->SrNo }}</td>
                    <td>{{ $item->ProfessorsName}}</td>
                    <td>{{ $item->Subject}}</td>
                    <td>{{ $item->Qualification }}</td>
                    <td>{{ $item->TeachingExperience }}</td>
                    <td>{{ $item->ProfessionalExperience}}</td>
                    <td>{{ $item->Research_Publication}}</td>
                    <td><img src="{{$item->Photo}}" height="100" width="100"></td>
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditTeachingStaffAdjunct',array($item->SrNo))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewTeachingStaffAdjunct',array($item->SrNo))}}">| View |</a>
...
@section('selectize-script')
    ...
    <script type="text/javascript">
        function changestatus(SrNo, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/TeachingStaffAdjunctStatusChange')}}/" + SrNo + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
