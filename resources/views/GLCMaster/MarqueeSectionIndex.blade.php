...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-4">
                {{--                <h5 class="text-primary nav-link active">Marquee Details</h5>--}}
                <h5 class="">Marquee Details</h5>
            </div>
            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreateMarqueeDetails') }}"
                   style="color:white;">
                    {{--                    <b class="text-primary border-red-light">Create</b> </a>--}}
                    <b class="">Create</b> </a>
            </div>
        </div>
    </div>
    ...
    <div class="table-responsive card" style="padding:10px;">
        <table id="dt" class="table table-bordered" style=" border: 1px solid gainsboro; color: gainsboro; font-size: medium;
        width:100%; text-align: center;">
            ...
            <tbody>
            @foreach($MarqueeSectionList as $item)
                <tr>
                    <td id="pi" scope="row">{{ $item->id }}</td>
                    <td>{{ $item->Text}}</td>
                    <td>{{ $item->PdfUrl }}</td>
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditMarqueeDetails',array($item->id))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewMarqueeDetails',array($item->id))}}">| View |</a>

                        @if( $item-> IsActive == '1' )
                            <a class="text-primary" href="#" onclick="changestatus('{{$item->id}}','0')">DeActive</a>
                        @Else
                            <a class="text-primary" href="#" onclick="changestatus('{{$item->id}}','1')">Active</a>
                        @endif
...
@section('selectize-script')
    ...
    <script type="text/javascript">
        function changestatus(id, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/MarqueeStatusChange')}}/" + id + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
