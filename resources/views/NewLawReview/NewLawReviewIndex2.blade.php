...
<div class="" style="margin: 50px; padding:10px;" id="TableOut">
    ...
    <div class="">
        <div class="row">
            <div class="col-md-8">
                <h5 class="text-primary nav-link active">New Law Review Details</h5>
            </div>

            <div class="col-md-3" style="float: right; margin: 20px;">
                <a class="bttn" href="{{ URL::to('/CreateNewLawReview') }}"
                   style="color:white;">
                    <b class="">Create</b> </a>
            </div>
        </div>
    ...
    <div class="table-responsive card" style="padding:10px;">
        ...
        width:100%; text-align: center;">
            ...
            <tbody>
            @foreach($NewLawReviewList as $item)
                <tr>
                    <td id="pi" scope="row">{{ $item->Title}}</td>
                    <td>{{ $item->Description}}</td>
                    <td>{{ $item->IsActive }}</td>
                    <td>
                        <a class="text-primary" href="{{ URL::to('EditNewLawReview',array($item->SrNo))}}">Edit</a>
                        <a class="text-primary" href="{{ URL::to('ViewNewLawReview',array($item->SrNo))}}">| View |</a>
...
@section('selectize-script')
    ...
    <script type="text/javascript">
        function changestatus(SrNo, IsActive) {
            ...
            if (confirm(msg)) {
                $.ajax({
                    url: "{{URL::to('/NewLawReviewChange')}}/" + SrNo + "/" + IsActive,
                    type: 'GET',
                    datatype: "json",
                    traditional: true,
                    success: function (data) {
                        debugger;
                        window.location.href = '/NewLawReviewIndex2';
                    },
                    error: function (ex) {
                        console.log(ex);
                    }
